...
class Recharge_daily extends CI_Controller
{
	...
	public function lists($provider = 'highchart')
	{
		...
		if(!empty($serverId) && !empty($startTime))
		{
			...
			$sql = "SELECT FROM_UNIXTIME(`funds_time`, '%k') as `hour`, SUM(`funds_amount`) as `amount` FROM `funds_checkinout` WHERE `server_id`='{$serverId}' AND `funds_flow_dir`='CHECK_IN' AND `funds_time`>={$startTime} AND `funds_time`<={$endTime} {$partner} AND `appstore_status`=0 GROUP BY `hour`";
