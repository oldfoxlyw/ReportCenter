...
$(function() {
	...
	$.post("<?php echo site_url('account/reflow_account/lists/highchart'); ?>", {
	...
	for(var i in json)
	{
		if(i != "axis")
		{
			...
			for(var j in json[i])
			{
				if(json[i][j])
				{
					rowData[json.axis.indexOf(j) + 1] = parseInt(json[i][j].reflow_account);
				}
				else
				{
					rowData[json.axis.indexOf(j) + 1] = "-";
				}
				data[json.axis.indexOf(j)] = parseInt(json[i][j].reflow_account);
			}
			aaData.push(rowData);
			obj.data = data;
			series.push(obj);
