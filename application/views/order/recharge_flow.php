...
$(function() {
	...
	$("#btnSearch").click(function() {
		...
		$('#listTable').empty();
		$.post("<?php echo site_url('order/recharge_flow/lists/highchart'); ?>", {
...
function onData(data) {
	...
	var aaData = [];
	for(var i in json) {
		var row = [json[i].account_guid, json[i].account_nickname, json[i].account_level, json[i].funds_amount / 100];
		aaData.push(row);
	}
